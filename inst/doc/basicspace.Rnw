...
This R (R Development Core Team, 2011) package contains software designed to recover latent dimensions (i.e. a basic space)
...
as Saiegh (2009) and Abrajano and Poole (2011).  The principal purpose of the package is to facilitate
...
has no missing data. In each cell $x_{ij}$, respondent $i$ (\textit{i=1, \ldots, n}) reports their position on
issue scale $j$ (\textit{j=1, \ldots, m}), with some responses missing.\footnote{The `0' subscript
...
of interest can all be recovered using singular value decomposition. To see why this is true, set $c$ equal
to the column means of $X$, or $c_j = \frac{\sum\limits_{i=1}^n x_{ij}}{n} = \bar{x}_j$.  Then the
...
\noindent where $U$ is an $n$ by $m$ matrix, $\Lambda$ is an $m$ by $m$ diagonal matrix of singular
values, and $V$ is an $m$ by $m$ orthogonal matrix such that $U'U=V'V=VV'=I_{m}$, where $I_{m}$
is the $m$ by $m$ identity matrix. This implies that in the absence of missing data, one solution for
$\Psi$ and $W$ is:
...
the square roots of $\Lambda$. The solution shown in (\ref{two}) has rank $m$. If a lower rank solution
is desired, $s<m$, then Eckart and Young (1936) have shown that the least squares approximation in $s$ dimensions
...
In the presence of missing data in matrix $X_0$, the use of singular value decomposition
...
\noindent where $m_{i}$ is the number of entries in the $i$th row. In minimizing this objective function, two
constraints from the earlier analysis in the absence of missing data are applied. First, we exploit the the fact
that $\Psi$ and $W$ are orthogonal matrices, which implies that $\Psi'\Psi = W'W$.\footnote{More specifically,
$\Psi'\Psi = \Lambda^{0.5}U'U\Lambda^{0.5} = \Lambda^{0.5}I_m\Lambda^{0.5} = W'W = \Lambda^{0.5}V'V\Lambda^{0.5} = \Lambda^{0.5}I_{m}\Lambda^{0.5} = \Lambda.$} Secondly,
following our earlier restriction that $J_n'[X-J_nc'] = 0_m'$, $J_n'U = J_n'\Psi = 0_m'$ as well.
...
\noindent where $n_{j}$ is the number of entries in the $j$th column. Let $W^*$ be an $m_i$ by $s$ matrix with
appropriate rows corresponding to missing entries in $X_0$ removed, $x_{0i}$ be the
...
$\hat{c}$, and $\hat{\Psi}$. Once a set of starting values have been generated, (\ref{six}) and (\ref{seven}) are
...
If two or more dimensions are to be estimated, start values for other dimensions can be generated by replacing
the data matrix $X_0$ with the matrix of residuals $E_{0k}$ in equations (\ref{nine}) and (\ref{ten}). However, no further estimation
of start values for $\hat{c}$ is required. The matrix of residuals to be used for generating start values
on dimension $k$ is:

$$E_{0k} = X_0 - \displaystyle\sum\limits_{l=1}^{k} \hat{\Psi}_l\hat{w}_l\mathrm{'} - J_n\hat{c}'$$

This residual matrix allows the generation of higher-dimension start values by iterating $\Gamma$ to
maximize the positive elements in $E_{0k}$. Equations (\ref{six}) and (\ref{seven}) become, respectively:

%Equation 12 in paper
\begin{equation}
\hat{\psi}_{ik} = \frac{\displaystyle\sum\limits_{j=1}^{m_i} \hat{w}_{jk}e_{1(k-1)ij}}{\sum\limits_{j=1}^{m_j} \hat{w}_{jk}^2}
\label{eleven}
\end{equation}

%Equation 11 in paper
\begin{equation}
\hat{w}_{jk} = \frac{\displaystyle\sum\limits_{i=1}^{n_j} \hat{\psi}_{ik}e_{(k-1)ij}}{\sum\limits_{i=1}^{n_j} \hat{\psi}_{ik}^2}
\label{twelve}
\end{equation}
...
and $\hat{\psi}_{i1}$ by using equations (\ref{eight}) to (\ref{ten}) and then iterating 
equations (\ref{six}) and (\ref{seven}) until convergence. In the second stage, if the
number of dimensions to be estimated $k > 1$, higher dimensional starting values for $\hat{\psi}_{ik}$ and $\hat{w}_{ik}$
are generated dimension by dimension using equations (\ref{nine}) and (\ref{ten}) and then iterating
equations (\ref{eleven}) to (\ref{twelve}) until convergence. Finally, the starting values generated 
in the preceding two stages are improved by iterating equations (\ref{six}) to (\ref{seven}) until convergence.
...
frames are \textbf{stimuli}, which contain estimates of $\hat{W}$ and $c$, as well as $\textbf{individuals}$,
...
Similarly, we can repeat the same procedure for $W$ which is shown in Figure \ref{fig:six}. In Figure
\ref{fig:eight} we repeat this analysis for $c$, the vector of column means estimated with one dimension.  In
both cases the estimates for $\hat{W}$ and $\hat{c}$ are a monotonic transformation of the
...
The transposed basic space model is a generalization of the original model by Aldrich and McKelvey (1977)
to matrices with missing values and estimates with more than one dimension. For historical purposes, we include
the original Aldrich-McKelvey estimator with this package, and estimate the same model using it
as shown below. While the \emph{aldmck()} function accepts nearly identical arguments to the
\emph{blackbox\_transpose()}, one notable difference appears by default. In addition to ranking
of stimuli, \emph{aldmck()} also accepts a column (specified by the argument \emph{respondent})
in the data matrix that specifies the respondent's self placements on the issue scale.
The reported respondent rating is then transformed into an ideology score by applying the respondent's
personal stretch and bias parameters to that score.
...
\bibitem{Abrajano} Abrajano, Marisa and Keith T. Poole (2011). ``A Method of Linking Surveys Using Affective
Signatures with an Application to Racial/Ethnic Groups in the U.S. In \emph{Who Gets Represented}, edited
by Peter Enns and Christopher Wlezien. New York: Russell Sage Foundation.
